namespace Networking
{
    ...
    public class DomainLoader_old
    {
        ...
        public DomainLoader_old(string url, IEnumerable<IDataWriter> writers, IWebClientFactory clientFactory, int maxThread = 10)
            :this(url, Enumerable.Empty<string>(), writers, clientFactory, maxThread)
        {

        }

        public DomainLoader_old(string url, IEnumerable<string> parsedUrls, IEnumerable<IDataWriter> writers, IWebClientFactory clientFactory, int maxThread = 10)
        {
            _writers = writers;
